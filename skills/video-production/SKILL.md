...
description: Turn recordings into finished video packages with speech cleanup, horizontal or vertical edits, thumbnails, and platform titles and descriptions. Use for requests to edit footage, make social versions, or package an existing video for publication.
license: MIT
metadata:
  author: SterlingChin
  version: "1.0.0"
...
Run the complete production job after the output formats are clear. A package includes the requested video versions, matching thumbnails, and a title plus description/caption for every selected platform. Do not wait for the user to request the packaging pieces separately.
...
7. Carry forward caption/overlay preferences. Avoid covering faces or useful screen content. For `both-same`, preserve approved audio when only the picture changes. For `both-distinct`, review the short's meaning independently.
...
- Save a combined copy document and clean per-platform files. Update the manifest with actual video, cover and copy paths, titles and descriptions.
...
Run the package helper's `check` command. Missing covers or platform titles/descriptions mean the package is unfinished. Fix omissions or report the specific blocker without claiming completion. The helper does not judge narrative quality or perform publication.

Show finished media, link the copy package, and state runtime/format. Keep the manifest and edit instructions available. During revisions, update changed items and affected dependencies; reuse assets that still match.
